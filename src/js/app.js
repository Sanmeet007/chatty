    ...
    response => {
      ...
      response.forEach(docs => {
        ...
        if (docs.type == "added" || docs.type == "modified") {
          ...
          yesterday = yesterday.getFullYear() +"-"+ parseInt(yesterday.getMonth() + 1)+"-"+  parseInt(yesterday.getDate() - 1);
          ...
          if (document.querySelector("[doc='"+ docs.doc.id+"']") == null) {
            if (thisMessenger.messageDates.includes(currentDate)) {
              ...
              if (currentDate === today) {
                thisMessenger.innerHTML +=
                `
                <div slot="message" class="date">Today</div>
                <list-element
                doc="${docs.doc.id}"
                read="${data.read}"
                slot="message"
                ...
                thisMessenger.innerHTML +=
                `
                <div slot="message" class="date">Yesterday</div>
                <list-element
                doc="${docs.doc.id}"
                read="${data.type}"
                slot="message"
